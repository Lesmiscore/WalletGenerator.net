...
function ReedSolomonDecoder(field) {
  ...
  this.decode = function(received, twoS) {
    ...
    for (var i = 0; i < twoS; i++) {
      ...
      var eveval = poly.evaluateAt(this.field.exp(dataMatrix ? i + 1 : i));
      syndromeCoefficients[syndromeCoefficients.length - 1 - i] = eveval;
      if (eveval != 0) {
        noError = false;
      }
    }
    ...
    if (navigator.getUserMedia) {
      ...
      navigator.getUserMedia(
        ...
        function(error) {
          ...
          _video = null;
          if (error && error.message) _error(error.message);
          else if (error && error.name) _error(error.name);
          else _error(error);
        }
      );
      ...
      qrcode.callback = function(data) {
        if (data && data.indexOf("error") != 0) {
          stop();
          if (data.indexOf("bitcoin:") == 0) data = data.substring(8);
          _success(data);
        }
      };
    } else {
